...
class BigNum(object):
  ...
  def slow_mul(self, other):
    ...
    for i in range(Ni):
        carry = Byte.zero()
        for j in range(Nj):
            digit = self.d[i] * other.d[j] 
            digit += C.d[i + j + 1].word() 
            digit += carry.word()
            C.d[i + j + 1] = digit.lsb()
            carry = digit.msb()
        C.d[i + Nj] = carry
  ...
  def fast_mul(self, other):
    ...
    result_high_high = self_high * other_high #Z_2
    result_low = self_low * other_low # Z_0
    result_high = (self_low + self_high) * (other_low + other_high) - \
                  (result_high_high + result_low) # Z_1
  ...
  def slow_divmod(self, other):
    ...
    S = BigNum(other.d)
    s_list = [S]
    N = max( len(other.d), len(self.d) )
    Q = BigNum.zero(N)
    R = BigNum(self.d)
    i = 0
    while len(s_list[-1].d) <= N and s_list[-1] <= self:
        i += 1
        new = s_list[-1] + s_list[-1]
        s_list.append(new)
